    ...
    public function getDataAKIAKB()
    {
        ...
        return DataTables::of(EpidemiPenyakit::with(['penyakit', 'desa'])->get())
            ->addColumn('aksi', function ($row) {
                $data['edit_url'] = auth()->user()->can('access.data.epidemi_penyakit.edit') ? route('data.epidemi-penyakit.edit', $row->id) : null;
                $data['delete_url'] = auth()->user()->can('access.data.epidemi_penyakit.delete') ? route('data.epidemi-penyakit.destroy', $row->id) : null;
