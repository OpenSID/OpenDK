...
class ProfilRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'kode_pos'                        => 'required|integer|max:12',
            'telepon'                         => 'required|integer|max:15',
            'email'                           => 'required|email',
            'tahun_pembentukan'               => 'required|integer',
            'dasar_pembentukan'               => 'required|regex:/^[a-zA-Z0-9 \.\-\/]+$/|max:50',
            'nama_camat'                      => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
            'sekretaris_camat'                => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
            'kepsek_pemerintahan_umum'        => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
            'kepsek_kesejahteraan_masyarakat' => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
            'kepsek_pemberdayaan_masyarakat'  => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
            'kepsek_pelayanan_umum'           => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
            'kepsek_trantib'                  => "required|regex:/^[a-zA-Z '\.,\-]+$/|max:150",
