<div class="modal fade" id="modal-form">
    <div class="modal-dialog">
        <form action="{{ route('setting.tipe-regulasi.store') }}" method="post">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">Tambah Tipe Regulasi</h4>
                </div>
    ...
    <script>
        $(function() {
            $('#modal-form').on('click', 'button[type="submit"]', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        form[0].reset();
                        $('#data-tipe-regulasi').DataTable().ajax.reload();
            ...
            $(document).on('click', '.open_form', function() {
                ...
                const routeEdit = '{{ route('setting.tipe-regulasi.edit', ':id') }}'.replace(':id', id);
                const routeUpdate = '{{ route('setting.tipe-regulasi.update', ':id') }}'.replace(':id', id);
                ...
                $.get(routeEdit)
                    .done(data => {
                        ...
                        $('#modal-form .modal-title').text('Ubah Tipe Regulasi');
