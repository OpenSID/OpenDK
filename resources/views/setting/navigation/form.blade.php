<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        {!! Html::text('name', null, ['class' => 'form-control', 'required' => true, 'id' => 'name']) !!}
        {!! Html::hidden('parent_id', $parent_id, ['class' => 'form-control', 'required' => true, 'id' => 'parent_id'])
        !!}
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        <select name="type" id="type" class="form-control select2" required>
            ...
            <optgroup label="Profil">
                ...
                <option value="1" data-value="{{ 'profil/' . $key }}" @selected('profil/' .
                    $key==optional($navigation)->url))>{{ $value }}</option>
                @endforeach
            </optgroup>
            <optgroup label="Desa">
                @foreach (\App\Enums\MenuTipe::getDesa() as $key => $value)
                @php $nama_desa = ucwords($key . ' ' . $value); @endphp
                <option value="2" data-value="{{ 'desa/' . Str::slug($nama_desa) }}" @selected('desa/' .
                    Str::slug($nama_desa)==optional($navigation)->url)>{{ $nama_desa }}</option>
                @endforeach
            </optgroup>
            <optgroup label="Statistik">
                @foreach (\App\Enums\MenuTipe::getStatistik() as $key => $value)
                <option value="3" data-value="{{ 'statistik/' . $key }}" @selected('statistik/' .
                    $key==optional($navigation)->url)>{{ $value }}</option>
                @endforeach
            </optgroup>
            <optgroup label="Potensi">
                @foreach (\App\Enums\MenuTipe::getPotensi() as $key => $value)
                <option value="3" data-value="{{ 'potensi/' . $key }}" @selected('potensi/' .
                    $key==optional($navigation)->url)>{{ $value }}</option>
                @endforeach
            </optgroup>
            <optgroup label="Unduhan">
                @foreach (\App\Enums\MenuTipe::getUnduhan() as $key => $value)
                <option value="4" data-value="{{ 'unduhan/' . $key }}" @selected('unduhan/' .
                    $key==optional($navigation)->url)>{{ $value }}</option>
                @endforeach
            </optgroup>
            <optgroup label="Publikasi">
                @foreach (\App\Enums\MenuTipe::getPublikasi() as $key => $value)
                <option value="4" data-value="{{ 'publikasi/' . $key }}" @selected('publikasi/' .
                    $key==optional($navigation)->url)>{{ $value }}</option>
                @endforeach
            </optgroup>
...
<div class="form-group" id="view-url">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        {!! Html::text('url', null, ['class' => 'form-control', 'required' => true, 'id' => 'url', 'value' =>
...
<div class="form-group">
    ...
    <div class="col-md-2 col-sm-2 col-xs-12">
        {!! Html::select('status', ['1' => 'Ya', '0' => 'Tidak'], 1, ['class' => 'form-control', 'required' => true,
        'id' => 'status']) !!}
    </div>
</div>
