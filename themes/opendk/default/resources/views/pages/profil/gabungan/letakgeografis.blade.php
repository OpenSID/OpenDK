    ...
    @if (!empty($profil))
        ...
        <div class="col-md-8 col-sm-8">
            ...
            <div class="box-footer">
                <p style="text-align: justify">{{ ucwords(strtolower($sebutan_wilayah . ' ' . $profil->nama_kecamatan . ', Kabupaten ' . $profil->nama_kabupaten . ', Provinsi ' . $profil->nama_provinsi)) }} mempunyai <b> Luas {{ number_format($profil->dataumum->luas_wilayah_value) }} km<sup>2</sup></b>
                    yang mencakup<b> {{ $wilayah_desa->count() }} Desa/Kelurahan</b>. Adapun <b> {{ terbilang($wilayah_desa->count()) }} Desa/Kelurahan </b> tersebut yaitu :
                <ul>
                    @foreach ($wilayah_desa as $desa)
                        <li>Desa {{ $desa->nama_desa }}</li>
                    @endforeach
                </ul>
                </p>
