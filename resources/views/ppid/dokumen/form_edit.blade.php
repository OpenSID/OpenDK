...
<div class="form-group">
    ...
    <div class="col-sm-10">
        {!! html()->select('id_jenis_dokumen', $jenis_dokumen->pluck('nama', 'id')->toArray(), $dokumen->id_jenis_dokumen)
            ->class('form-control')
...
<div class="form-group">
    ...
    <div class="col-sm-10">
        @if($dokumen->file)
        <p class="help-block">File saat ini: <strong>{{ $dokumen->file }}</strong></p>
        @endif
        {!! html()->file('file')->class('form-control')->accept('pdf,doc,docx,xls,xlsx,zip,rar') !!}
        <p class="help-block">Format yang diperbolehkan: PDF, DOC, DOCX, XLS, XLSX, ZIP, RAR. Maksimal 10MB. Kosongkan jika tidak ingin mengubah file.</p>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-10">
        <div class="radio">
            <label>
                <input type="radio" name="status" value="1" {{ $dokumen->status ? 'checked' : '' }}> Aktif
            </label>
            &nbsp;
            <label>
                <input type="radio" name="status" value="0" {{ !$dokumen->status ? 'checked' : '' }}> Tidak Aktif
            </label>
        </div>
    </div>
</div>
