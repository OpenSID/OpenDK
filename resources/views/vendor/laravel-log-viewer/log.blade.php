            ...
            <div class="box-body no-padding">
                <ul class="nav nav-pills nav-stacked">
                    @foreach ($files as $file)
                        <li @if ($current_file == $file) class="active" @endif><a
                                href="?l={{ \Illuminate\Support\Facades\Crypt::encrypt($file) }}"> {{ $file }}
                            </a></li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="box-header with-border">
                <div class="p-3">
                    @if ($current_file)
                        ...
                        <a class="btn btn-social btn-sm btn-info visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"
                            id="clean-log"
                        ...
                        <a class="btn btn-social btn-sm btn-danger visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"
                            id="delete-log"
                        ...
                        @if (count($files) > 1)
                            <a id="delete-all-log"
                                class="btn btn-social btn-sm btn-danger visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"
                ...
                <div class="col-sm-12">
                    ...
                    @else
                        <div class="table-responsive">
                            <table id="table-log" class="table table-striped table table-bordered dataTable table-hover"
                                ...
                                <tbody>
                                    @foreach ($logs as $key => $log)
                                        <tr data-display="stack{{ $key }}">
                                            ...
                                            @endif
                                            <td class="date">
                                                {{ \Carbon\Carbon::parse($log['date'])->translatedFormat('d-m-Y H:i') }}
                                            </td>
                                            <td class="text">
                                                @if ($log['stack'])
