...
$(function() {
    ...
    function renderArtikel(items) {
        ...
        return items.map(function(single) {
            ...
            var tanggal = item.tanggal_terbit ? new Date(item.tanggal_terbit).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '';
    ...
    $('#list_desa').change(function() { applyFilter(); });
    $('#tanggal').change(function() { applyFilter(); });
    $('#btn-cari').click(function() { applyFilter(); });
    $('#cari').on('keypress', function(e) { if (e.which === 13) applyFilter(); });
