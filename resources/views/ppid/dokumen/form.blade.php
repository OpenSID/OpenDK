...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        <div class="radio">
            <label>
                {!! html()->radio('tipe_dokumen', $key, old('tipe_dokumen', 'file'))
                ->required() !!} {{ $label }}
            </label>
        </div>
        @endforeach
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        <div class="btn-group status-toggle" data-toggle="buttons">
            <label class="btn btn-success {{ old('status', 'terbit') === 'terbit' ? 'active' : '' }}">
                <input type="radio" name="status" value="terbit"
                       {{ old('status', 'terbit') === 'terbit' ? 'checked' : '' }}
                       required> Terbit
            </label>
            <label class="btn btn-danger {{ old('status') === 'tidak_terbit' ? 'active' : '' }}">
                <input type="radio" name="status" value="tidak_terbit"
                       {{ old('status') === 'tidak_terbit' ? 'checked' : '' }}> Tidak
            </label>
        </div>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        {!! html()->date('tanggal_publikasi', old('tanggal_publikasi'))
