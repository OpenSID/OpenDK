                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 pl-md-1">
                                    {!! html()->select('bulan')->options([
                                            ...
                                            '11' => 'November',
                                            '12' => 'Desember',
                                        ])->class('form-control')->placeholder('- Pilih Bulan -')->value(old('bulan', isset($dokumen) ? $dokumen->bulan : '')) !!}
                                </div>

                                {{-- Filter Tahun --}}
                                <div class="col-md-4 pr-md-1">
                                    {!! html()->select('tahun')->options(
                                            collect(range(date('Y'), date('Y') - 10))->mapWithKeys(fn($y) => [$y => $y])->toArray(),
                                        )->class('form-control')->placeholder('- Pilih Tahun -')->value(old('tahun', isset($dokumen) ? $dokumen->tahun : '')) !!}
                                </div>

                                {{-- Filter Jenis Dokumen --}}
                                <div class="col-md-4 pr-md-1">
                                    {!! html()->select('jenis_dokumen_id')->options(\App\Models\JenisDokumen::pluck('nama', 'id')->toArray())->class('form-control')->placeholder('- Pilih Jenis Dokumen -')->id('jenis_dokumen_id')->value(old('jenis_dokumen_id', isset($dokumen) ? $dokumen->jenis_dokumen_id : '')) !!}
                                </div>
                            </div>
                        </div>
                    </div>
