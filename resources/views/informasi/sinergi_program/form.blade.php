...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        <input type="file" name="gambar" id="gambar" accept="jpg,jpeg,png" class="form-control" {{ $sinergi ? ''
            : 'required' }}>
        <br>

        <img src="@if (isset($sinergi->gambar)) {{ asset($sinergi->gambar) }} @else {{ 'http://placehold.co/1000x600' }} @endif"
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        {!! Html::select('status', ['1' => 'Aktif', '0' => 'Tidak Aktif'])->value(old('status', isset($sinergi)->value(old('status', isset($sinergi) ? $sinergi->status : '')) ? $sinergi->status : '')))->class('form-control') !!}
    </div>
</div>
